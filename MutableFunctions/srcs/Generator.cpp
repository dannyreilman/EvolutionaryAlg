...
static bool StartLine(std::istream& terms, std::ostream& output, Generator::Workspace& space);
 ...
 */
static unique_ptr<EvaluateToDouble> InterpretObject(std::istream& terms, Generator::Workspace& space, bool copy, const string& term);
static unique_ptr<EvaluateToDouble> InterpretObject(std::istream& terms, Generator::Workspace& space, bool copy);
...
static void PrintCommand(std::istream& terms, Generator::Workspace& space);

//Works same as print but evaluates each term instead of printing each term
static void EvalCommand(std::istream& terms, Generator::Workspace& space);

//Generates an input variable with arg1 and gives it the value Eval(arg2), discarding extra terms
static void InputCommand(std::istream& terms, Generator::Workspace& space);

//Works similar to eval and print but calls Mutate on each term instead
static void MutateCommand(std::istream& terms, Generator::Workspace& space);

//Calls commands from a file as if they were passed through the command line
static void BatchCommand(std::istream& terms, std::ostream& output, Generator::Workspace& space);
...
static void ExportCommand(std::istream& terms, Generator::Workspace& space);
 ...
 */
static void SaveOrMove(std::istream& terms, Generator::Workspace& space, bool copy);
static void SaveCommand(std::istream& terms, Generator::Workspace& space)
{
	SaveOrMove(terms, space, true);
}

//Same as save, except objects change ownership instead of being cloned
static void MoveCommand(std::istream& terms, Generator::Workspace& space)
{
 ...
 */
static unique_ptr<EvaluateToDouble> GenerateCommand(std::istream& terms, Generator::Workspace& space, bool copy);

unique_ptr<EvaluateToDouble> Generator::CopyObject(string name)
{
	if(name.compare("ans") == 0)
	{
		return space.ans->Clone();
	}
	return space.variables[name]->Clone();
}
	
void Generator::LoadMutationValues(string filename)
{
	//Load from defaultMutationValues.txt
	std::ifstream valueReader(filename);

	MutationOptions::ReadInput(space.options, valueReader);
	
	valueReader.close();	
}

bool Generator::Run(std::istream& terms)
{
	return StartLine(terms, *output, space);
}
...
static bool StartLine(std::istream& terms, std::ostream& output, Generator::Workspace& space)
{
	...
	try{
		while(lineParse >> term)
		{
			...
			{
				output << "A command line interface for working with MutableFunc objects and for testing various elements manually" << endl;
				output << "Valid commands are:" << endl;
				output << "quit: ends the program" << endl;
				output << "print [object(s)]: prints objects later in the line" << endl;
				output << "eval [object(s)]: Prints the evaluation of objects later in the line" << endl;
				output << "save [object] [variable]: Copies an object into a variable" << endl;
				output << "move [object] [variable]: Moves an object into a variable (object will no longer be accessible if it is a variable)" << endl;
				output << "[object]: Stores [object] in ans (equivalent to move [object] ans" << endl;
				output << "---" << endl;
				output << "[object] can be a variable, gen or combine command" << endl;
				output << "gen [function] [object1] [object2]: generates a functionEvaluator object with copies of object1 and object2 as arguments" << endl;
				output << "combine [function] [object1] [object2]: same as gen except objects are moved over" << endl;
			}
			...
			{
				BatchCommand(lineParse, output, space);
			}
...
static void BatchCommand(std::istream& terms, std::ostream& output, Generator::Workspace& space)
{
	...
	{
		StartLine(batchReader, output, space);
	}
}
